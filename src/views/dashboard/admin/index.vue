            ...
            <div v-for="(row, index) in props.row.qa_mark_list" :key="props.row._id + index" class="answer-content">
              ...
              <div class="answer-operation">
                <el-button
                  :disabled="!row.answer"
                  type="success"
                  @click="crudStore.QAMarkRecord('reasonable', props.row, row)"
                  >合理</el-button
                >
                <el-input v-model="row.my_answer" placeholder="请输入修改内容" style="width: 200px; margin: 0 10px" />
                <el-button type="info" @click="crudStore.QAMarkRecord('unreasonable', props.row, row)"
                  >不合理</el-button
                >
                <el-button type="warning" @click="crudStore.QAMarkRecord('uncertain', props.row, row)"
                  >不确定</el-button
                >
              </div>
            </div>
